...
import './EditViewLesson.css';
import logo1 from '../../../AddSchedule/img/img1.png';
import logo2 from '../../../AddSchedule/img/img2.png';
...
class EditLesson extends React.Component {
    ...
    delit = () => {
        ...
        let newLesson = this.state.lesson;
        newLesson.delit = true;
        this.props.saveLessonGlobal(newLesson);//тут косяк подгружает после номер дня, скорее всего дело в том, что удаленные сохраняються тоже, но не отображаються
    }


    componentDidMount() {
        if (this.props.discription != undefined) {
            this.getDiskripsion();
            this.setState({
                klick: true,
            })
    ...
    getDiskripsion = () => {
        this.setState({
            lesson: {
                numbreDay: this.props.numbreDay,
                numbreLesson: this.props.numbreLesson,
                item: this.props.discription.item,
                view: this.props.discription.view,
                teacher: this.props.discription.teacher,
                place: this.props.discription.place,
                time_start_hour: this.props.discription.time_start_hour,
                time_start_minute: this.props.discription.time_start_minute,
                time_stop_hour: this.props.discription.time_stop_hour,
                time_stop_minute: this.props.discription.time_stop_minute,
            }
        })
        let lesson = {
            numbreDay: this.props.numbreDay,
...
export default connect(mapStateToProps, { dataLesson, saveLessonGlobal })(EditLesson)